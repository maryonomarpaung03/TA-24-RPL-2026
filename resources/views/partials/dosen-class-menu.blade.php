...
<div class="relative" x-data="{ menuOpen: false, modal: @json($openCreateClassModal ? 'create' : null) }" @keydown.escape.window="menuOpen = false; modal = null">
    ...
    <div x-show="modal === 'create'"
        ...
        <div class="bg-white rounded-xl shadow-lg w-full max-w-lg p-6 border border-slate-200" @click.stop>
            <div class="flex items-center justify-between mb-4 border-b pb-3">
                <h3 class="font-bold text-gray-800 text-sm uppercase">Buat Kelas Baru</h3>
                <button type="button" @click="modal = null" class="text-gray-400 hover:text-gray-600">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <form method="POST" action="{{ route('dosen.classes.store') }}" class="space-y-4 max-h-[70vh] overflow-y-auto pr-1"
                  onsubmit="var j=this.querySelector('[name=jurusan]');if(j)j.disabled=false;">
                @csrf

                @if($errors->any())
                <div class="rounded bg-red-50 border border-red-200 text-red-800 px-3 py-2 text-xs">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" placeholder="Informasi singkat tentang kelas (opsional)"
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        <label class="flex items-start gap-3 rounded-lg border p-3 cursor-pointer hover:bg-gray-50 {{ old('visibility', 'public') === 'public' ? 'border-blue-400 bg-blue-50/50' : 'border-gray-200' }}">
                            <input type="radio" name="visibility" value="public" class="mt-0.5" @checked(old('visibility', 'public') === 'public')>
                            <span>
                        ...
                        <label class="flex items-start gap-3 rounded-lg border p-3 cursor-pointer hover:bg-gray-50 {{ old('visibility') === 'closed' ? 'border-blue-400 bg-blue-50/50' : 'border-gray-200' }}">
                            <input type="radio" name="visibility" value="closed" class="mt-0.5" @checked(old('visibility') === 'closed')>
                            <span>
